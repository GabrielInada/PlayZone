...
import {
  ...
  SIGNUP_ROUTE,
  PUNICOES_ROUTE,
  VALIDAR_SUMULA_ROUTE,
} from "@/constants/routes";

// ── Mock ──────────────────────────────────────────────────────────────────────
const mockStats = {
  matchesToday:         5,
  activeChampionships: 23,
  pendingSummaries:    12,
};
      ...
      <div className="max-w-6xl mx-auto w-full space-y-3 p-1 sm:px-6 lg:px-3">
        ...
        <div className="mt-4 shadow-md">
          <AlertBanner
            ...
            href={VALIDAR_SUMULA_ROUTE}
          />
        </div>

        <section className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <StatCard title="Partidas de Hoje"      value={mockStats.matchesToday}         icon={IconCalendar} />
          <StatCard title="Campeonatos Ativos"     value={mockStats.activeChampionships}  icon={IconTrophy} />
        </section>

        <section className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <ActionCard title="Estádios Cadastrados"   description="Gerencie estádios, adicione, edite ou exclua" href={ESTADIOS_ROUTE} />
          <ActionCard title="Cadastrar Novo Usuário" description="Crie novos perfis de usuários"                href={SIGNUP_ROUTE} />
          <ActionCard title="Validar Cadastro"       description="Valide novos usuários do sistema"             href="#" />
          <ActionCard title="Validar Ocorrências"    description="Valide possíveis punições enviadas"           href={PUNICOES_ROUTE} />
        </section>
